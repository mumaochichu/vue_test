<template>
  ...
  >
    <h2 class="vl-notice-title" @mousedown="moveStart">
      ...
      <!-- <template v-if="options.maxmin == true">
        <span class="lv-icon-maxmini">
          ...
          <template v-else-if="maxMiniState === 1">
            <i class="vlayer vlicon-huanyuan lv-icon-huanyuan" @click="maxmini"></i>
          </template>
      ...
      <div class="lv-icon-maxmini">
        <template v-if="options.maxmin == true && maxMiniState === 0">
          <div
            class="vlayer vlicon-maxmin-icon vlicon-mini lv-icon-mini"
            @click="mini"
          ></div>
          <div
            class="vlayer vlicon-maxmin-icon vlicon-max lv-icon-max"
            @click="max"
          ></div>
        </template>
        <template v-else-if="options.maxmin == true && maxMiniState === 1">
          <div
            class="vlayer vlicon-maxmin-icon vlicon-huanyuan lv-icon-huanyuan"
            @click="maxmini"
          ></div>
        </template>
        <template v-else-if="options.maxmin == true && maxMiniState === 2">
          <div
            class="vlayer vlicon-maxmin-icon vlicon-huanyuan lv-icon-huanyuan"
            @click="maxmini"
          ></div>
        </template>
        <div class="vlicon-maxmin-icon icon-remove" @click="close"></div>
      </div>
    </h2>
    <slot></slot>
    <span
...
export default {
  ...
  methods: {
    ...
    mini() {
      ...
      let width = window.innerWidth - 145 + 'px'
      this.addStyle = {
        overflow: 'hidden',
        // bottom: 0,
        // left: 250 * domMinIndex + 135 + "px",
        // top: "auto",
        top: 100 + 48 * domMinIndex + 'px',
        left: width,
        bottom: 'auto',
        width: '100px',
        height: '42px',
    ...
    maxmini() {
      //还原
      let dom = document.getElementById(this.options.id)
  ...
  mounted() {
    ...
    //     'vl-notify vl-notify-main vl-notify-alert vl-notify-iframe'
    //   ),
    //   (element) => {
    //     _this.$nextTick(() => {
    //       //监听到事件后执行的业务逻辑
